...
bios_add(void)
{
    ...
    if (/*AT && */ cpu_s) {
        ...
        temp_is386ex      = (cpu_f->package == CPU_PKG_386EX);
    }
    ...
    if (temp_is6117) {
        ...
        mem_set_mem_state_both(biosaddr | 0x03f00000, biosmask + 1,
                               MEM_READ_ROMCS | MEM_WRITE_ROMCS);
    } else if (temp_is386ex) {
        mem_mapping_add(&bios_high_mapping, biosaddr | 0x03400000, biosmask + 1,
                        bios_read, bios_readw, bios_readl,
                        NULL, NULL, NULL,
                        rom, MEM_MAPPING_EXTERNAL | MEM_MAPPING_ROM | MEM_MAPPING_ROMCS, 0);

        mem_set_mem_state_both(biosaddr | 0x03400000, biosmask + 1,
                               MEM_READ_ROMCS | MEM_WRITE_ROMCS);
        mem_set_mem_state_both(0 | 0x03400000, biosmask + 1,
                               MEM_READ_ROMCS | MEM_WRITE_ROMCS);
    } else if (temp_is286) {
